...
def _yn(prompt: str) -> bool:
    while True:
        a = input(prompt + " (j/n): ").strip().lower()
...
def voeg_recept_toe_interactief() -> Recept:
    ...
    # Ingrediënten (incl. kcal en optioneel plantaardig alternatief)
    while _yn("Ingrediënt toevoegen?"):
        ...
        kcal = _float("kcal per eenheid: ", 0.0)
        ing = Ingredient(in_naam, hoeveelheid, eenheid, kcal)
...
# =========================
#   FR-7: Verwijderen + bevestiging
# =========================
def bevestig_verwijderen(recept_naam: str) -> bool:
    print(f"Je staat op het punt '{recept_naam}' te verwijderen.")
    return _yn("Weet je het zeker?")

# =========================
#   FR-2/3/4 + tonen details
# =========================
def toon_recept_interactief(recepten: list[Recept], idx: int) -> None:
    if idx < 0 or idx >= len(recepten):
        print("Ongeldige keuze."); return
    ...
    # Verwijderflow met bevestiging
    if _yn("Dit recept verwijderen?"):
        if bevestig_verwijderen(gekozen.get_naam()):
...
def kies_recept_index(recepten: list[Recept]) -> int | None:
    if not recepten: return None
    keuze = _int("Kies een receptnummer: ", 1)
    if keuze > len(recepten):
        print("Ongeldige keuze."); return None
    return keuze - 1

# =========================
#   Seed: 3 startrecepten (FR-1)
# =========================
def init_startrecepten() -> list[Recept]:
    ...
    r1.voeg_stap_toe(Stap("Bak de kip en ui en voeg kerriepoeder toe.", tip="Scheutje water tegen aanbakken."))
    ...
    r3.voeg_stap_toe(Stap("Klop de eieren los met peper en zout.", tip="Scheutje water maakt het luchtiger."))
